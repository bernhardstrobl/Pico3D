...
void logic_shoot() {
    ...
    for (int i = 0; i < MAX_ZOMBIES; i++) {
        ...
        if (distancex < ZOMBIE_SHOOT_DISTANCE && distancey < ZOMBIE_SHOOT_DISTANCE) {
            if (logic_raycast(zombie_list[i].x, FIXED_POINT_FACTOR * 4, zombie_list[i].y, FIXED_POINT_FACTOR / 2, FIXED_POINT_FACTOR, FIXED_POINT_FACTOR / 2, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2)) {
                
                //single hit will destroy balloon
                //play pop animation and increase kill count
